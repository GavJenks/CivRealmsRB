...
public class GrowthConfig {
	...
	public double getRate(Block block) {
		...
		if (soilMaxLayers > 0) {
			...
			int soilCount = 0;
            int airForgivenessCount = 0;
            int offset = 1;
            //pre-check the offset for air, no taking advantage!
            while (offset < soilLayerOffset){
                if (block.getRelative(0,-offset,0).getType().equals(Material.AIR)){
                    airForgivenessCount++;
                    if (airForgivenessCount > 5){
                        return rate;
                    }
                }
                offset++;
            }
            //now allow up to 5 air in the main body minus however were found in the pre check
			while (soilCount < soilMaxLayers) {
                if (newBlock.getType().equals(Material.AIR) && airForgivenessCount < 6){
